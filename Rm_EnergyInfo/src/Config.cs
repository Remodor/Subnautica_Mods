...
public class Config : ConfigFile
{
    ...
    [Slider("Sample Interval", 0.05f, 5.5f, DefaultValue = 1.0f, Format = "{0:F2}", Step = 0.05f, Tooltip = "Defines the refresh rate of the energy info. Shorter intervals mean quicker updates but increase fluctuation. Some energy consumers/ producers, like the filtration machine or the thermal plant, draw/ produce energy in intervals. Increasing the sample interval can cover these spikes and calculate an energy consumption per in-game hour (e/h). When consumer and producer spikes overlap (filtration machine + thermal plant), they can interfere and show wrong values. Further increasing the interval can correctly show the e/h. Longer intervals can be set inside the config file."), OnChange("OnConfigChange")]
    public float SampleInterval = 1.0f;

    [Slider("Spike Threshold", 0.05f, 5.5f, DefaultValue = 0.4f, Format = "{0:F2}", Step = 0.05f, Tooltip = "Some actions draw energy on a per-use basis, like the prawn suit's jump ability consuming 1.2 energy per jump. These spikes can be shown separately without affecting the consumption-per-hour rate. The spike threshold defines what is considered a spike. Greater thresholds can be set inside the config file."), OnChange("OnConfigChange")]
    ...
    [Toggle("Draw Energy Unit", Tooltip = "Draws energy units -- 'e/h' for energy per in-game hour and 'e' for energy spikes."), OnChange("OnConfigChange")]
